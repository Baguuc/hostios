...
impl Client {
    ...
    pub async fn execute(self: &Self, query: crate::fql::statement::Statement) -> Result<QueryExecuteResult, QueryExecuteError> {
        use crate::fql::statement::Statement;
            ...
            Statement::ReadDir(path) => {
                ...
                while let Some(entry) = entries.next_entry().await.map_err(|_| QueryExecuteError::Fs(String::from("cannot read entry")))? {
                    ...
                    if path.is_file() {
                        ...
                        let path = crate::models::Path::parse(path).unwrap();
                        
                        entries_parsed.push(crate::models::Entry::File(path));
                    }
                    else if path.is_dir() {
                        let root_str = self.root
                            .to_string_lossy()
                        ...
                        let path = crate::models::Path::parse(path).unwrap();
                        entries_parsed.push(crate::models::Entry::Directory(path));
                    }
                }
...
pub enum QueryExecuteResult {
    Null,
    Bool(bool),
    EntryList(Vec<crate::models::Entry>),
    String(String)
}
...
impl QueryExecuteResult {
    ...
    pub fn unwrap_entry_list(self) -> Vec<crate::models::Entry> {
        return match self {
